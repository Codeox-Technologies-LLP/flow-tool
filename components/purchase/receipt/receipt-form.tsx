...
export function ReceiptForm({
    ...
    mode === "edit" && receipt
      ? {
        ...
        deliveryDate: receipt.deliveryDate
          ? receipt.deliveryDate.split("T")[0]
          : "",
      }
      : undefined;
  ...
  const onSubmit = async (data: ReceiptFormData) => {
    try {
      ...
      if (mode === "create") {
        ...
        if (response.status) {
          ...
          if (response.receiptId) {
            router.push(`/flow-tool/purchase/receipt/${response.receiptId}`);
          } else {
            router.push("/flow-tool/purchase/receipt");
          }
        } else {
        ...
        const response = await receiptApi.edit(receiptId, data);
      ...
      console.error(
        `Error ${mode === "create" ? "creating" : "updating"} receipt:`,
        error
      );
    } finally {
      setLoading(false);
  ...
  const handleCancel = () => {
    router.push("/flow-tool/receipt/receipt-orders");
  };
