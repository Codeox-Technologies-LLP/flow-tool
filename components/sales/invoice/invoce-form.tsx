...
export function InvoiceForm({ mode, invoice, invoiceId }: InvoiceFormProps) {
    ...
    const {
        ...
        defaultValues: mode === "edit" && invoice ? {
            ...
            billingAddress: invoice.billingAddress ? "billing" : "",
            deliveryAddress: invoice.deliveryAddress ? `${invoice.deliveryAddress.street}-${invoice.deliveryAddress.city}-${invoice.deliveryAddress.state}-${invoice.deliveryAddress.zip}` : "",
            dealId: invoice.dealId || "",
            quotationId: invoice.quotationId || "",
            expiryDate: invoice.expiryDate || "",
            status: invoice.status || "draft",
            products: invoice.products || [{ product: "", productName: "", qty: 1, rate: 0, discount: 0, tax: 0, description: "" }],
        } : {
            relatedTo: "",
            contactId: "",
            assignedTo: "",
    ...
    const onSubmit = async (data: InvoiceFormData) => {
        try {
            ...
            const response = mode === "create"
                ...
                : await invoiceApi.edit(invoiceId!, payload);

            if (response.status) {
                toast.success(`Invoice ${mode === "create" ? "created" : "updated"} successfully!`);
                mode === "create" ? router.push("/flow-tool/sales/invoices") : router.refresh();
            } else {
                toast.error(`Failed to ${mode} Invoice`, { description: response.message });
            }
        } catch (error) {
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
            <Card>
                <CardHeader>
                    ...
                    <CardDescription>
                        {mode === "create" ? "Enter Invoice details and select client" : "Update the Invoice details below"}
                    </CardDescription>
                </CardHeader>
            ...
            <div className="flex justify-end gap-3">
                <Button type="button" variant="outline" onClick={() => router.push("/flow-tool/sales/invoices")} disabled={loading}>
                    Cancel
                </Button>
