...
import { DeliveryFormData, deliverySchema } from "@/lib/validations/delivery";
...
export function DeliveryForm({ mode, delivery, deliveryId }: DeliveryFormProps) {
    ...
    const {
        ...
        defaultValues: mode === "edit" && delivery ? {
            relatedTo: delivery.relatedTo || "",
            contactId: delivery.contactId || "",
            assignedTo: delivery.assignedTo || "",
            locationId: delivery.locationId || "",
            billingAddress: delivery.billingAddress ? "billing" : "",
            deliveryAddress: delivery.deliveryAddress ? `${delivery.deliveryAddress.street}-${delivery.deliveryAddress.city}-${delivery.deliveryAddress.state}-${delivery.deliveryAddress.zip}` : "",
            dealId: delivery.dealId || "",
            quotationId: delivery.quotationId || "",
            expiryDate: delivery.expiryDate || "",
            status: delivery.status || "draft",
            products: delivery.products || [{ product: "", productName: "", qty: 1, rate: 0, discount: 0, tax: 0, description: "" }],
        } : {
            relatedTo: "",
            contactId: "",
            assignedTo: "",
    ...
    const onSubmit = async (data: DeliveryFormData) => {
        try {
            ...
            const response = mode === "create"
                ...
                : await deliveryApi.edit(deliveryId!, payload);

            if (response.status) {
                toast.success(`Delivery ${mode === "create" ? "created" : "updated"} successfully!`);
                mode === "create" ? router.push("/flow-tool/sales/deliveries") : router.refresh();
            } else {
                toast.error(`Failed to ${mode} Delivery`, { description: response.message });
            }
        } catch (error) {
            toast.error(`Failed to ${mode} Delivery`, {
                description: error instanceof Error ? error.message : "An error occurred",
            });
            console.error(`Error ${mode}ing Delivery:`, error);
        } finally {
            setLoading(false);
        }
    };
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
            <Card>
                <CardHeader>
                    ...
                    <CardDescription>
                        {mode === "create" ? "Enter Delivery details and select client" : "Update the Delivery details below"}
                    </CardDescription>
                </CardHeader>
            ...
            <Card>
                <CardHeader>
                    ...
                    <CardDescription>Add products to this Delivery</CardDescription>
                </CardHeader>
                <CardContent>
            ...
            <div className="flex justify-end gap-3">
                <Button type="button" variant="outline" onClick={() => router.push("/flow-tool/sales/deliveries")} disabled={loading}>
                    Cancel
                </Button>
